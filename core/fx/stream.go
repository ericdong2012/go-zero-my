...
import (
	"go-zero-my/core/collection"
	"go-zero-my/core/lang"
	"go-zero-my/core/threading"
	"sort"
	"sync"
...
type (
	...
	// ForAllFunc defines the method to handler all elements in a Stream.
	ForAllFunc func(pipe <-chan interface{})
	// ForEachFunc defines the method to handler each element in a Stream.
	...
	// ParallelFunc defines the method to handler elements parallelly.
...
// Walk lets the callers handler each item, the caller may write zero, one or more items base on the given item.
